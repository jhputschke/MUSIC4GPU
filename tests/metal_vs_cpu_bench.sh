#!/usr/bin/env bash
# metal_vs_cpu_bench.sh — Compare Metal GPU vs CPU builds of MUSIC.
#
...
# which needs no external data files.
...
check_bins() {
    ...
    if [[ ! -x "${CPU_BIN}" ]]; then
        ...
        echo "  Build it with:"
        echo "    cmake -S . -B build && cmake --build build -j$(sysctl -n hw.logicalcpu)"
        ok=0
    fi
    if [[ ! -x "${GPU_BIN}" ]]; then
        echo "ERROR: Metal binary not found: ${GPU_BIN}"
        echo "  Build it with:"
        echo "    cmake -S . -B build_metal -DUSE_METAL=ON && cmake --build build_metal -j$(sysctl -n hw.logicalcpu)"
        ok=0
    fi
    [[ ${ok} -eq 1 ]]
}

# Write a Gubser-viscous input file with given grid dimensions.
# All grids are boost-invariant (1 eta slice) so wall time scales as Nx*Ny.
make_input() {
    local nx=$1 ny=$2 neta=$3 tau_end=$4 outfile=$5
    local fm_x fm_y fm_eta
    fm_x=$(python3 -c "print(${nx}*0.1)")
    fm_y=$(python3 -c "print(${ny}*0.1)")
    fm_eta=$(python3 -c "print(${neta}*0.1)")
    cat > "${outfile}" <<EOF
echo_level  6
mode  2
Initial_profile  0
...
# Usage: elapsed=$(run_timed BIN INPUT LOGFILE ERRFILE)
run_timed() {
...
# Compare eps_max traces between two log files; print max relative error.
compare_eps() {
...
def parse_eps(path):
    ...
    for line in open(path):
        m = re.search(r'eps_max\s*=\s*([\d.e+\-]+)', line)
        if m:
...
if not cpu or not gpu:
    print("  (no eps_max lines found — check echo_level)")
    sys.exit(0)
...
if max(diffs) < 1e-4:
    ...
    print("  FAIL  (discrepancy > 1e-4 — check kernel correctness)")
PYEOF
}
...
echo "========================================================"
echo " MUSIC Metal GPU vs CPU benchmark"
echo " $(date)"
echo "========================================================"
...
# Print Metal device info: run a 2x2x1 grid for a single step to capture init msgs.
PROBE_INP="${TMPDIR_BENCH}/probe_input.dat"
cat > "${PROBE_INP}" <<'PROBE_EOF'
echo_level  1
Initial_profile  0
boost_invariant  1
Grid_size_in_x  2
Grid_size_in_y  2
Grid_size_in_eta  1
X_grid_size_in_fm  0.2
Y_grid_size_in_fm  0.2
Eta_grid_size  0.1
Initial_radius_size_in_fm  2.6
Initial_time_tau_0  1.0
Total_evolution_time_tau  0.05
Delta_Tau  0.005
EOS_to_use  0
Viscosity_Flag_Yes_1_No_0  1
Include_Shear_Visc_Yes_1_No_0  1
...
EndOfData
PROBE_EOF
METAL_STDERR="${TMPDIR_BENCH}/metal_probe.err"
"${GPU_BIN}" "${PROBE_INP}" > /dev/null 2> "${METAL_STDERR}" || true
DEVICE=$(grep "Metal device:" "${METAL_STDERR}" 2>/dev/null || true)
[[ -n "${DEVICE}" ]] && echo "GPU        : ${DEVICE#*Metal device: }"

# ── test cases ───────────────────────────────────────────────────────────────
#  Format: "label  Nx  Ny  Neta  tau_end"
#  Delta_Tau=0.005 matches typical MUSIC production runs.
#  tau_end=0.5 → 100 steps; stays well away from the freeze-out plateau
#  where float32 accumulation would cause CPU and GPU to freeze at
#  different times (a non-kernel artifact unrelated to GPU correctness).
declare -a CASES=(
    "32x32x1    32   32   1   0.5"
    "64x64x1    64   64   1   0.5"
    "128x128x1  128  128  1   0.5"
)
...
for case_str in "${CASES[@]}"; do
    ...
    # Suppress echo for cleaner timing (echo_level 0 is already set)
    ...
    # Quick max-rel-error from eps_max lines
...
PYEOF
    ...
    RESULTS_TABLE+="${label}|${t_cpu}|${t_gpu}|${speedup}|${max_err}\n"
done

# ── correctness detail (use largest case for detail) ─────────────────────────

...
void Advance::init_metal_if_needed(SCGrid &arena_current) {
    ...
    // GPU EOS tables are sampled at rhob=0, so only zero-muB EOS are valid on
    // the GPU path.  Route finite-muB EOS (flag_muB==true: EOSQ, neos, BEST,
    // UH) to the CPU; all muB=0 EOS (idealgas, s95p, WB, hotQCD incl. 91)
    // continue on the GPU.
    if (eos.get_flag_muB()) {
        music_message << "[MUSIC-GPU] finite-muB EOS (whichEOS="
                      << DATA.whichEOS << "), using CPU.";
